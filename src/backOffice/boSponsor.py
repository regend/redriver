...
class BoSponsor(unittest.TestCase):
    def setUp(self):
    ...
    def testBoSponsor(self):
        ...
        driver.find_element_by_id('8079').click()
        ...
        member_placementNo = driver.find_element_by_id('memberDetail_'+str(member_id)).get_attribute('memberplacementmemberno')
        ...
        placement_no = Oradao.sqlDiy(Oradao(),
                                  'select * from mm_member m where m.id=(select m.placement_id from mm_member m where m.member_no=\''+member['MEMBER_NO']+'\')')
        self.assertEqual(member_placementNo, placement_no['MEMBER_NO'][0])
        self.assertEqual(member_grade, transData('member.enrollmentGrade').get(member['ENROLLMENT_GRADE']))

        # 判断当前会员是否有推荐下线,有则对下线编号进行断言
        if member_son > '0':
            # 数据库取安置下线
            son_no = Oradao.sqlDiy(Oradao(),
                                     'select * from mm_member m where m.sponsor_id=(select m.id from mm_member m where m.member_no=\''+member['MEMBER_NO']+'\') order by m.member_no asc')
